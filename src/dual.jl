...
@inline Dual{T}(value, partials::SparseVector) where {T} = isempty(partials) ?
    Dual{T}(value, Partials{0,typeof(value)}(partials)) : Dual{T}(value, Partials(partials))
...
@inline function Dual{T}(value, partial1, partials...) where {T}
    @show value, partial1, partials
    @show typeof(sparse(vcat(partial1, [partials...])))
    return Dual{T}(value, SparseVector(vcat(partial1, [partials...])))
end
